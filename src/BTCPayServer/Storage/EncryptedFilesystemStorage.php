...
class EncryptedFilesystemStorage implements StorageInterface
{
    ...
    public function load($id)
    {
        ...
        $decoded = openssl_decrypt(\BTCPayServer\Util\Util::binConv($encoded), self::METHOD, $this->password, 1, self::IV);

        if (false === $decoded) {
            $decoded = openssl_decrypt(\BTCPayServer\Util\Util::binConv($encoded), self::METHOD, $this->unencoded_password, 1, self::IV);
        }
