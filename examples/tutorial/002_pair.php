...
$config = [
    ...
    'btcpay_host' => 'https:<some_host>:<some_port>',
    'pairing_code' => '<some_pairing_code_from_btcpayserver>',
    // An integration with BTCPay server is typically the "merchant" facade.
    'facade' => 'merchant',
    'token_label' => '<some_label_from_bcpayserver>',
];
...
if ($config['facade'] == 'merchant' || $config['facade'] == 'payroll'):
    try {
        ...
        $url = $config['btcpay_host'] . '/api-access-request?pairingCode=' . $pairingCode;
        ...
        /**
         * NOTE: The `(string)` is included so that the objects are converted to a
         * user friendly string.
         */
        exit(1); // We do not want to continue if something went wrong
    }
